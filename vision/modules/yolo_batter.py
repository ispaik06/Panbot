# Panbot/vision/modules/yolo_batter.py
...
class YOLOBatterInfer:
    """
    step(frame_bgr) -> (triggered, vis, info)
    - triggered: stable trigger (hold_frames consecutive)
    - vis: visualization frame (warped or full, overlay included)
    - info: dict with ratio, mask_area, hit_count...
    """

    def __init__(self, cfg: YOLOBatterConfig):
        self.cfg = cfg
        ...
        if cfg.use_warp:
            ...
            if cfg.warp_w > 0 and cfg.warp_h > 0:
                self.warp_out_w, self.warp_out_h = int(cfg.warp_w), int(cfg.warp_h)
                auto = False
            else:
                self.warp_out_w, self.warp_out_h = compute_warp_size_from_corners(self.corners)
                auto = True
            print(f"[YOLO] warp ON corners={cfg.corners_path} size={self.warp_out_w}x{self.warp_out_h} (auto={auto})")
        else:
            print("[YOLO] warp OFF (full frame)")
    ...
    def step(self, frame_bgr: np.ndarray):
        ...
        if self.cfg.use_warp:
            frame = warp_topview(frame_bgr, self.corners, self.warp_out_w, self.warp_out_h)
        else:
            frame = frame_bgr
        ...
        if r.masks is not None and r.masks.data is not None and len(r.masks.data) > 0:
            ...
            masks_bool = masks > float(self.cfg.mask_thr)
            ...
            triggered = self.stable_trigger

            vis = overlay_mask(frame, largest)

        text = f"[YOLO] area={mask_area:.0f}px ratio={ratio:.3f} thr={self.cfg.area_thr_ratio:.3f} hold={self.hit_count}/{self.cfg.hold_frames} TRIGGER={triggered}"
        ...
        info: Dict[str, Any] = {
            ...
            "thr": float(self.cfg.area_thr_ratio),
